...
- **Sessions can live on a server**, and that cost almost nothing because a relay
  ...
  transport, no join-code change, and a player cannot tell the difference. Three
  secrets, and keeping them straight is the whole design: the **`host_token`** is
  in the join code and everyone has it; the **`gm_token`** claims the GM's *seat*
  and goes to one person (a wrong one is **refused**, never quietly downgraded to
  a player — that failure "works" right until a hidden roll reaches the table);
  the **admin secret** lives in the server's `/etc` and opens the *catalog*,
  which is the entirety of "only a GM can create a session". A remote GM needed
  ...
  message existed). `SessionCatalog` answers *every* control request — returning
  the whole catalog rather than a delta makes a stale list impossible.
  `core/session/hub_client.py` is the app's side, deliberately unlike `client.py`:
  no reader thread, no events — connect, ask, read, close.
