"""The player's half of a session: keeping the GM's copy of the sheet current.

A joined player pushes a character snapshot on join and on every change, so the
GM's card and the read-only sheet behind it are live rather than a stale copy
from join time.
